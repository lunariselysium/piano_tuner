...
@njit(cache=True)
def numba_cost_function(all_frequencies, intervals_array, targets_array, b_values):
    """
    ...
    for i in range(num_intervals):
        ...
        # B values are pre-calculated and looked up by index
        B1 = b_values[idx1]
        B2 = b_values[idx2]
...
# Create mappings from MIDI note to array index and vice-versa
...
# Convert intervals list to a NumPy array of indices for fast access in Numba
intervals_array_list = []
targets_list = []
for n1, n2, p1, p2, weight in intervals_to_check:
    # Use indices instead of MIDI numbers
    intervals_array_list.append([midi_to_idx_map[n1], midi_to_idx_map[n2], p1, p2, weight])
    
    # Calculate and store the corresponding target beat rate
...
# Pre-calculate B values for all notes
...
# Initial guess from the "perfect" reference tuning
initial_guess = np.array([reference_tuning[m] for m in variable_midi_notes])

# Create arrays holding the indices for fixed and variable notes
...
# Create a template array to quickly build the full frequency list
...
# This wrapper handles the conversion between the optimizer's view (variable_freqs)
# and the Numba function's view (all_frequencies).
def cost_function_wrapper(variable_freqs):
    ...
    """
    # Create the full array of frequencies for this iteration
    ...
    return numba_cost_function(current_all_freqs, intervals_array, targets_array, b_values_array)
...
# First, run the cost function once to allow Numba to compile it.
...
minimizer_kwargs = {
    "method": "L-BFGS-B",
    "jac": False, # We are not providing a Jacobian function
